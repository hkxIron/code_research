...
class GOperation(GBaseClass):
    ...
    def partialGradient(self, partial):
        ...
        if self.operatorType == "pow":
            ...
            return GOperationWrapper(self.left.partialGradient(partial), part3, "multiple")  #multiple parital derviate

        if self.operatorType == "exp":
            return GOperationWrapper(self.left.partialGradient(partial), self, "multiple") #exp(f(x))*df(x)
